...
class ProviderCallbackLogsScreen extends StatefulWidget {
  ...
  @override
  State<ProviderCallbackLogsScreen> createState() => _ProviderCallbackLogsScreenState();
}

class _ProviderCallbackLogsScreenState extends State<ProviderCallbackLogsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) setState(() => _error = 'Provider callback logs could not be loaded.');
    } finally {
  ...
  Future<void> _markProcessed(ProviderCallbackLogItem item) async {
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Callback marked processed.')));
      await _load();
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Callback status could not be updated.')));
    }
  }

  @override
  Widget build(BuildContext context) {
    final failed = _items.where((item) => item.status.toLowerCase() == 'failed').length;
    final invalidSignatures = _items.where((item) => item.signatureValid == false).length;
    return Scaffold(
      appBar: AppBar(
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            Text('Callback operations', style: Theme.of(context).textTheme.headlineMedium),
            ...
            if (_loading && _items.isEmpty)
              ...
              Row(children: [
                Expanded(child: B2BMetricCard(label: 'Callbacks', value: '${_items.length}', icon: Icons.webhook_outlined)),
                const SizedBox(width: B2BSpacing.sm),
                Expanded(child: B2BMetricCard(label: 'Failed', value: '$failed', icon: Icons.error_outline_rounded)),
              ]),
              const SizedBox(height: B2BSpacing.sm),
              B2BMetricCard(label: 'Invalid signatures', value: '$invalidSignatures', icon: Icons.gpp_bad_outlined),
              const SizedBox(height: B2BSpacing.lg),
              if (_items.isEmpty)
    ...
    return Padding(
      ...
      child: B2BSurface(
        child: Column(
          ...
          children: [
            Row(children: [
              Expanded(child: Text(item.provider.isEmpty ? 'Unknown provider' : item.provider, style: const TextStyle(fontWeight: FontWeight.w900))),
              Text(item.status, style: TextStyle(color: item.status.toLowerCase() == 'failed' ? AppColors.danger : AppColors.textSecondary)),
            ]),
            ...
            if (item.signatureValid != null) ...[
              ...
              Text(
                ...
                style: TextStyle(color: item.signatureValid! ? AppColors.success : AppColors.danger),
              ),
            ],
            if (item.errorMessage.isNotEmpty) ...[
              const SizedBox(height: B2BSpacing.xs),
              Text(item.errorMessage, style: const TextStyle(color: AppColors.danger)),
            ],
