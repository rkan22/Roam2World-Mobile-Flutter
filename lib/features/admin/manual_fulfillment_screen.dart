...
class ManualFulfillmentScreen extends StatefulWidget {
  ...
  @override
  State<ManualFulfillmentScreen> createState() =>
      _ManualFulfillmentScreenState();
}
...
class _ManualFulfillmentScreenState extends State<ManualFulfillmentScreen> {
  ...
  Future<void> _addSims() async {
    ...
    final iccids = raw
        ...
        .toList();
    await _run(
      () => _repository.addBlankSims(iccids),
      'SIM inventory updated.',
    );
  }
  ...
  Future<void> _assignQr(ManualTaskItem task) async {
    ...
    await _run(
      () => _repository.assignQr(task.taskId, code: code),
      'QR delivery completed.',
    );
  }
  ...
  Future<void> _activateSim(ManualTaskItem task) async {
    final reference = await _ask(
      ...
      'Supplier activation reference (optional)',
    );
  ...
  Future<void> _cancel(ManualTaskItem task) async {
    ...
    await _run(
      () => _repository.cancelTask(task.taskId, reason: reason),
      'Task cancelled.',
    );
  }
              ...
              Row(
                children: [
                  Expanded(
                    child: B2BMetricCard(
                      label: 'Blank SIMs',
                      value: '${data.availableBlankStock}',
          ...
          B2BSurface(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: B2BSpacing.xs),
                Text(
                  '${task.orderNumber} · ${task.productType.toUpperCase()}',
                  ...
                  Wrap(
                    ...
                    children: [
                      if (task.canSendToFlexnet)
                        OutlinedButton.icon(
                          onPressed: () => _run(
                            () => _repository.sendToFlexnet(task.taskId),
                            'Sent to Flexnet.',
                          ),
    ...
    return Column(
      ...
      children: [
        FilledButton.icon(
          ...
          label: const Text('Add blank SIM ICCIDs'),
        ),
        const SizedBox(height: B2BSpacing.md),
        if (data.inventory.isEmpty)
          const ContentEmptyState(
            icon: Icons.sim_card_alert_outlined,
            title: 'No SIM inventory',
            message: 'Add blank physical SIM ICCIDs to the shared stock pool.',
          )
        else
          B2BSurface(
            padding: EdgeInsets.zero,
              ...
              children: [
                for (var i = 0; i < data.inventory.length; i++) ...[
                  ListTile(
                    ...
                    title: Text(
                      data.inventory[i].iccid,
                      style: const TextStyle(fontWeight: FontWeight.w800),
                    ),
                    subtitle: data.inventory[i].orderNumber.isEmpty
                        ? null
                        : Text(data.inventory[i].orderNumber),
