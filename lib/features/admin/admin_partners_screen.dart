...
class _AdminPartnersScreenState extends State<AdminPartnersScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    final visible = data?.items.where((item) {
      return query.isEmpty || item.companyName.toLowerCase().contains(query) || item.email.toLowerCase().contains(query) || item.resellerName.toLowerCase().contains(query);
...
class _Metric extends StatelessWidget { const _Metric({required this.icon, required this.label, required this.value}); final IconData icon; final String label; final String value; @override Widget build(BuildContext context) => Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 17), const SizedBox(width: 5), Text('$label: ', style: Theme.of(context).textTheme.bodySmall), Text(value, style: const TextStyle(fontWeight: FontWeight.w800))]); }

class _DealerDetailSheet extends StatefulWidget { const _DealerDetailSheet({required this.repository, required this.dealerId}); final AdminPartnersRepository repository; final int dealerId; @override State<_DealerDetailSheet> createState() => _DealerDetailSheetState(); }
...
class _ResellerDetailSheet extends StatefulWidget { const _ResellerDetailSheet({required this.repository, required this.resellerId}); final AdminPartnersRepository repository; final int resellerId; @override State<_ResellerDetailSheet> createState() => _ResellerDetailSheetState(); }
