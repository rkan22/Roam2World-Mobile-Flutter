...
class _OperationsCenterScreenState extends State<OperationsCenterScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _error = 'Operations data could not be loaded.');
      }
    } finally {
      if (mounted) {
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            const Text(
              'Failed orders, provider/API events and audit activity in one reseller workspace.',
              ...
              Row(
                children: [
                  Expanded(
                    child: B2BMetricCard(
                      label: 'Failed orders',
                      value: '${data.failedOrders.length}',
              ...
              SizedBox(
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) => ChoiceChip(
                    label: Text(
                      const ['Overview', 'Failed', 'API Logs', 'Audit'][index],
                    ),
                    selected: _tab == index,
              ...
              if (_tab != 0) ...[
                TextField(
                  ...
                  decoration: const InputDecoration(
                    hintText: 'Search operations data',
      ...
      children: [
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(
                      data.systemHealth.isOperational
                          ? Icons.check_circle_outline_rounded
                          : Icons.warning_amber_rounded,
                      color: data.systemHealth.isOperational
                          ? AppColors.success
                          : AppColors.warning,
                    ),
                    const SizedBox(width: B2BSpacing.sm),
                ...
                Wrap(
                  ...
                  children: [
                    _healthStatus('API', data.systemHealth.api),
                    _healthStatus('Database', data.systemHealth.database),
                    _healthStatus('Orders', data.systemHealth.orders),
                    _healthStatus('Wallet', data.systemHealth.wallet),
                    _healthStatus(
                      'Providers',
                      data.systemHealth.providerStatus,
                    ),
                  ],
                ),
              ],
            ...
            children: [
              Expanded(
                child: B2BMetricCard(
                  ...
                  value: '${data.activitySummary.users}',
              ...
              Expanded(
                child: B2BMetricCard(
                  label: 'Platform eSIMs',
                  value: '${data.activitySummary.esims}',
          ...
          Row(
            children: [
              Expanded(
                child: B2BMetricCard(
                  label: 'Resellers',
                  value: '${data.activitySummary.resellers}',
              ...
              Expanded(
                child: B2BMetricCard(
                  label: 'Dealers',
                  value: '${data.activitySummary.dealers}',
            ...
            child: Column(
              children: [
                ListTile(
                  ...
                  subtitle: const Text(
                    'Live reseller directory and active counts',
                  ),
                ...
                ListTile(
                  ...
                  subtitle: const Text(
                    'Live dealer directory and active counts',
                  ),
                  trailing: const Icon(Icons.chevron_right_rounded),
                ...
                ListTile(
                  ...
                  subtitle: const Text(
                    'Live admin pricing inventory and business totals',
                  ),
                  trailing: const Icon(Icons.chevron_right_rounded),
                ...
                ListTile(
                  ...
                  subtitle: const Text(
                    'Admin support queue and backend health',
                  ),
                  trailing: const Icon(Icons.chevron_right_rounded),
        ...
        B2BSurface(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: B2BSpacing.md),
              _overviewRow(
                'Failed provider/order queue',
                data.failedOrders.length,
                data.failedOrders.isEmpty
                    ? AppColors.success
                    : AppColors.danger,
              ),
              const Divider(height: B2BSpacing.xl),
              _overviewRow(
                'API / webhook events needing review',
                data.logs.where((item) => item.needsReview).length,
                AppColors.warning,
              ),
            ],
          ),
        ...
        if (recent.isEmpty)
          const ContentEmptyState(
            ...
            message: 'Live operational activity will appear here.',
          )
        else
  ...
  Widget _failed(List<FailedOrderItem> items) {
    ...
    final visible = items
        .where(
          (item) =>
              q.isEmpty ||
              '${item.provider} ${item.orderNumber} ${item.customer} ${item.packageName} ${item.status} ${item.error}'
                  .toLowerCase()
                  .contains(q),
        )
        .toList();
    if (visible.isEmpty) {
      return const ContentEmptyState(
        ...
        message: 'No live failed-order rows match this view.',
      );
    }
    return Column(
      children: [
          ...
          B2BSurface(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: B2BSpacing.xs),
                Text(
                  '${item.provider} · ${item.orderNumber}',
  ...
  Widget _logs(List<OperationLogItem> items) {
    ...
    final visible = items
        .where(
          (item) =>
              q.isEmpty ||
              '${item.provider} ${item.type} ${item.endpoint} ${item.method} ${item.statusCode} ${item.message}'
                  .toLowerCase()
                  .contains(q),
        )
        .toList();
    if (visible.isEmpty) {
      return const ContentEmptyState(
        icon: Icons.dns_outlined,
        title: 'No live log events',
        message:
            'Provider and webhook events will appear when backend log sources return data.',
      );
    }
    return Column(
      children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        '${item.method} ${item.endpoint.isEmpty ? 'Operational event' : item.endpoint}',
                        style: const TextStyle(fontWeight: FontWeight.w900),
                      ),
                      ...
                      const SizedBox(height: B2BSpacing.xs),
                      Text(
                        '${item.statusCode == 0 ? 'N/A' : item.statusCode} · ${item.durationMs}ms${_date(item.createdAt)}',
  ...
  Widget _audit(List<AuditEventItem> items) {
    ...
    final visible = items
        .where(
          (item) =>
              q.isEmpty ||
              '${item.actor} ${item.action} ${item.target} ${item.source} ${item.description}'
                  .toLowerCase()
                  .contains(q),
        )
        .toList();
    if (visible.isEmpty) {
      return const ContentEmptyState(
        ...
        message: 'No dedicated or composed activity matches this view.',
      );
    }
    return Column(
      children: [
  ...
  Widget _overviewRow(String label, int value, Color color) => Row(
    children: [
      Expanded(
  ...
  Widget _healthStatus(String label, String status) {
    final healthy = SystemHealthData.isHealthyStatus(status);
    return Container(
      ...
      decoration: BoxDecoration(
        color: (healthy ? AppColors.success : AppColors.warning).withValues(
          alpha: .1,
        ),
  ...
  String _date(DateTime? value, {String prefix = ' · '}) => value == null
      ? ''
      : '$prefix${DateFormat('dd MMM, HH:mm').format(value.toLocal())}';
}
