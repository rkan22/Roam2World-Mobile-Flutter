...
class DealerNetworkRepository {
  DealerNetworkRepository({ApiClient? apiClient}) : _apiClient = apiClient ?? ApiClient();
      ...
      parser: (response) {
        ...
        final rows = data is List
            ...
            : data is Map
                ...
                : const [];
        return rows is List
            ? rows.whereType<Map>().map((item) => DealerSummary.fromJson(Map<String, dynamic>.from(item))).toList()
            : const <DealerSummary>[];
      },
    );
    ...
    final requestsFuture = _apiClient.get<List<DealerFundingRequest>>(
      ...
      parser: (response) {
        ...
        return rows is List
            ? rows
                .whereType<Map>()
                .map((item) => DealerFundingRequest.fromJson(Map<String, dynamic>.from(item)))
    ...
    return _apiClient.get<List<DealerWalletTransfer>>(
      ...
      parser: (response) {
        ...
        final rows = data is List
            ? data
            : data is Map
                ? (data['results'] ?? data['transactions'] ?? data['wallet_transfers'] ?? const [])
                : const [];
        return rows is List
            ? rows.whereType<Map>().map((item) => DealerWalletTransfer.fromJson(Map<String, dynamic>.from(item))).toList()
            : const <DealerWalletTransfer>[];
      },
    );
  }

  Future<void> approveRequest(DealerFundingRequest request, {String? note}) async {
    await _apiClient.post<dynamic>(
      ApiEndpoints.mobileDealerWalletRequestApprove(request.id),
      data: {
        if (note != null && note.trim().isNotEmpty) 'notes': note.trim(),
        if (note != null && note.trim().isNotEmpty) 'reseller_notes': note.trim(),
      },
      parser: (data) => data,
    );
  }
