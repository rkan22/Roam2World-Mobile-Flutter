...
class _DealerNetworkScreenState extends State<DealerNetworkScreen> {
          ...
          child: Column(
            ...
            children: [
              ...
              SegmentedButton<bool>(
                ...
                selected: {credit},
                onSelectionChanged: submitting ? null : (value) => setSheetState(() => credit = value.first),
              ),
  ...
  Widget build(BuildContext context) {
    ...
    final networkBalance = data?.dealers.fold<double>(0, (sum, item) => sum + item.currentBalance) ?? 0;
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: B2BSpacing.xs),
            const Text('Manage dealer exposure, funding requests and wallet balances.', style: TextStyle(color: AppColors.textSecondary)),
            const SizedBox(height: B2BSpacing.lg),
            if (_loading)
              ...
              Row(children: [
                Expanded(child: B2BMetricCard(label: 'Dealers', value: '${data.dealers.length}', icon: Icons.storefront_outlined)),
                const SizedBox(width: B2BSpacing.sm),
                Expanded(child: B2BMetricCard(label: 'Active', value: '$active', icon: Icons.verified_user_outlined)),
              ]),
              const SizedBox(height: B2BSpacing.sm),
              Row(children: [
                Expanded(child: B2BMetricCard(label: 'Network balance', value: '$currency ${networkBalance.toStringAsFixed(0)}', icon: Icons.account_balance_wallet_outlined)),
                const SizedBox(width: B2BSpacing.sm),
                Expanded(child: B2BMetricCard(label: 'Pending approvals', value: '${data.pendingRequests.length}', icon: Icons.pending_actions_outlined)),
              ]),
                  ...
                  B2BSurface(
                    child: Row(children: [
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        if (request.dealerEmail.isNotEmpty) Text(request.dealerEmail, style: const TextStyle(color: AppColors.textSecondary)),
                        if (request.note.isNotEmpty) Text(request.note, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppColors.textSecondary)),
                      ])),
                      const SizedBox(width: B2BSpacing.sm),
                      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                        Text('${request.currency} ${request.amount.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.w900)),
                        const SizedBox(height: B2BSpacing.xs),
                        FilledButton.tonal(onPressed: () => _approve(request), child: const Text('Approve')),
                      ]),
                    ]),
                  ),
                  const SizedBox(height: B2BSpacing.sm),
              ...
              TextField(
                ...
                decoration: const InputDecoration(hintText: 'Search dealer, email or phone', prefixIcon: Icon(Icons.search_rounded)),
              ),
              const SizedBox(height: B2BSpacing.sm),
              SizedBox(
                height: 42,
              ...
              const SizedBox(height: B2BSpacing.lg),
              if (visible.isEmpty)
                const ContentEmptyState(icon: Icons.storefront_outlined, title: 'No dealers found', message: 'Try another search or status filter.')
              else
                    ...
                    child: Column(children: [
                      Row(children: [
                        Container(
                          ...
                          child: Text(_initials(dealer.name), style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.w900)),
                        ),
                        const SizedBox(width: B2BSpacing.sm),
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      const SizedBox(height: B2BSpacing.md),
                      Row(children: [
                        Expanded(child: _Metric(label: 'Available', value: '${dealer.currency} ${dealer.currentBalance.toStringAsFixed(2)}')),
                        Expanded(child: _Metric(label: 'Clients', value: '${dealer.totalClients}')),
                        Expanded(child: _Metric(label: 'Orders', value: '${dealer.totalOrders}', alignEnd: true)),
                      ]),
                    ]),
                  ),
                  const SizedBox(height: B2BSpacing.sm),
...
class _Status extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(active ? 'Active' : 'Suspended', style: TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.w900)),
    );
  }
}
