          ...
          child: ListView(
            ...
            children: [
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text('Physical SIM inventory', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w800)),
                          SizedBox(height: 4),
                          Text('Provider details are handled by the B2B backend.', style: TextStyle(color: Colors.white70)),
                        ],
                      ),
                    ),
                  ],
              ...
              if (_loading)
                ...
                ContentErrorState(message: _error!, onRetry: () => _load(refresh: true))
              else if (_packages.isEmpty)
                const ContentEmptyState(
                  icon: Icons.sim_card_outlined,
                  title: 'No SIM stock available',
                  message: 'There are currently no enabled physical SIM products.',
                )
              else ...[
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(B2BRadius.xl),
            border: Border.all(color: selected ? AppColors.primary : theme.colorScheme.outlineVariant, width: selected ? 2 : 1),
            ...
            children: [
              ...
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text('Minimum ${package.minimumQuantity}', style: theme.textTheme.labelMedium?.copyWith(fontWeight: FontWeight.w700)),
                ]),
              ),
              Text('${package.currency} ${package.price.toStringAsFixed(2)}', style: theme.textTheme.titleMedium?.copyWith(color: AppColors.primary)),
            ],
          ),
        ),
      ),
...
class _OrderPanel extends StatelessWidget {
  const _OrderPanel({required this.package, required this.quantity, required this.ordering, required this.onQuantityChanged, required this.onOrder});
  final SimCardPackage package;
  final int quantity;
  final bool ordering;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: theme.colorScheme.surface, borderRadius: BorderRadius.circular(B2BRadius.xl), border: Border.all(color: theme.colorScheme.outlineVariant), boxShadow: B2BShadows.card),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          const Text('Quantity'),
          const Spacer(),
          IconButton(onPressed: quantity > package.minimumQuantity ? () => onQuantityChanged(quantity - 1) : null, icon: const Icon(Icons.remove_circle_outline)),
        ...
        Row(children: [
          ...
          const Spacer(),
          Text('${package.currency} ${total.toStringAsFixed(2)}', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w900, color: AppColors.primary)),
        ]),
        const SizedBox(height: 14),
        SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: ordering ? null : onOrder, icon: ordering ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.local_shipping_outlined), label: Text(ordering ? 'Creating order...' : 'Place stock order'))),
      ]),
    );
  }
}
