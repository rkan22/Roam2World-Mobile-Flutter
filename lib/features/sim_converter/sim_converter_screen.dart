        ...
        child: ListView(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(
                  child: Text('Conversion history', style: Theme.of(context).textTheme.titleLarge),
                ),
                if (_workspace != null)
                  Text(
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: (result.valid ? AppColors.success : AppColors.warning).withValues(alpha: .25),
        ),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              Icon(
                result.valid ? Icons.verified_rounded : Icons.warning_amber_rounded,
                color: result.valid ? AppColors.success : AppColors.warning,
              ),
              const SizedBox(width: B2BSpacing.sm),
              Text(
                result.valid ? 'Valid activation data' : 'Activation data needs review',
                style: const TextStyle(fontWeight: FontWeight.w900),
              ),
            ],
          ),
